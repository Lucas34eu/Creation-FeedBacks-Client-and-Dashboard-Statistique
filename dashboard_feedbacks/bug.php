...
$questionsrequest = $bdd->query("select resolution,count(idBug) 
  from survey_bug
  group by resolution
order by count(idBug) desc
limit 20");
$tableau_resolution  = $questionsrequest->fetchAll(PDO::FETCH_ASSOC);


$questionsrequest = $bdd->query("SELECT count(*) FROM `survey_bug` ");
$total_entre = $questionsrequest->fetch(PDO::FETCH_ASSOC);
$debutsemaine = date('w');

//this week
$questionsrequest = $bdd->query("SELECT count(idBug)
  FROM `survey_bug` 
  WHERE date_creation 
  > date(now())");
...
//1 week ago
$questionsrequest = $bdd->query('SELECT count(idBug) FROM `survey_bug` 
...
//Url depart
$questionsrequest = $bdd->query('select count(idBug),url_depart 
from survey_bug
group by url_depart
order by count(idBug) desc
limit 10');
...
foreach ($url_depart as $compteur) {
    ...
    $total_entre10 += $compteur["count(idBug)"];
}
                                    ...
                                    <tbody>
                                        ...
                                        ?>
                                            <tr style="text-align:center">
                                                ...
                                                <td><?php echo $rows["count(idBug)"]; ?></td>

                                                <td>
                                                    <?php

                                                    echo $pourcentage = round($rows["count(idBug)"] * 100 / $total_entre["count(*)"]) . " %"
                            ...
                            <div class="card-body p-0">
                                <table class="table table-striped">
                                    ...
                                    <tbody>
                                        ...
                                        ?>
                                            <tr style="text-align:center">
                                                <td><?php echo $entry["count(idBug)"]; ?></td>
                                                <td><?php echo $pourcentage = round($entry["count(idBug)"] * 100 / $total_entre10) . " %";   ?></td>
                                    ...
                                    <tbody>
                                        ...
                                        ?>
                                            <tr style='text-align:center;'>
                                                ...
                                                <td><?php echo substr($entry2["naviguateur"], 0, 12) . "...";   ?></td>
                    ...
                    var myChart = new Chart(ctx, {
                        ...
                        data: {
                            ...
                            datasets: [{
                                ...
                                data: [
                                    <?php echo $countEntrer9WeekAgo[0]["count(idBug)"]; ?>,
                                    <?php echo $countEntrer8WeekAgo[0]["count(idBug)"] ?>,
                                    <?php echo $countEntrer7WeekAgo[0]["count(idBug)"] ?>,
                                    <?php echo $countEntrer6WeekAgo[0]["count(idBug)"] ?>,
                                    <?php echo $countEntrer5WeekAgo[0]["count(idBug)"]; ?>,
                                    <?php echo $countEntrer4WeekAgo[0]["count(idBug)"]; ?>,
                                    <?php echo $countEntrer3WeekAgo[0]["count(idBug)"]; ?>,
                                    <?php echo $countEntrer2WeekAgo[0]["count(idBug)"] ?>,
                                    <?php echo $countEntrer1WeekAgo[0]["count(idBug)"]; ?>,
                                    <?php echo $countEntrerThisWeek[0]["count(idBug)"] ?>
